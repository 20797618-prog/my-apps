// 账号体系 · 验证码邮件发送（QQ 邮箱 SMTP：465 隐式 TLS / 587 STARTTLS）
// 通过 Cloudflare Workers TCP socket (cloudflare:sockets) 实现最小 SMTP 客户端。
// 未配置 MAIL_PASS / MAIL_ENABLED!=='true' 时，由调用方（request-code）走开发模式直接返回验证码。
//
// 接入新项目时，只需在 Cloudflare Pages/Worker 的环境变量里配置以下项：
//   MAIL_ENABLED = "true"
//   MAIL_USER    = 发件邮箱（如 123@qq.com）
//   MAIL_PASS    = QQ 邮箱「授权码」（不是登录密码）
//   MAIL_PORT    = 465（推荐，隐式 TLS）或 587（STARTTLS）
//   MAIL_FROM_NAME = 你的应用名（会出现在邮件标题和发件人里）
...
export async function sendCode(env, to, code) {
    ...
    const appName = env.MAIL_FROM_NAME || '我的应用';
    ...
    try {
        ...
        const subject = '【' + appName + '】您的登录验证码：' + code;
        const text = '您的登录验证码是：' + code + '\r\n验证码 10 分钟内有效。若非本人操作请忽略本邮件。';
        const headers =
            'From: ' + encodeWord(appName) + ' <' + fromAddr + '>\r\n' +
            'To: <' + to + '>\r\n' +
